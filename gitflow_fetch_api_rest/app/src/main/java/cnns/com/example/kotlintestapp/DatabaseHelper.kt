...
class DataBaseHandler(var context: Context) : SQLiteOpenHelper(context, DATABASENAME, null,
    ...
    fun insertData(item: PokemonRecyclerView) {
...
//            Toast.makeText(context, "Bien ajouté dans la BD", Toast.LENGTH_SHORT).show()
    ...
    fun readData(limitMax: Int) : MutableList<PokemonRecyclerView> {
        val list: MutableList<PokemonRecyclerView> = ArrayList()
        val db = this.readableDatabase
        val query = "Select * from $TABLENAME ORDER BY id DESC LIMIT $limitMax"
        val result = db.rawQuery(query, null)
        if (result.moveToFirst()) {
            do {
                val item = PokemonRecyclerView("", "", 0)
